// From: https://github.com/xCss/Valine/blob/master/src/utils/detect.js
// Last edit date: 2019-10-30 03:03
...
export class Detect {
  ...
  constructor(u: string) {
    ...
    if (this.match['Mobile']) {
      this.match['Mobile'] = !(u.indexOf('iPad') > -1)
    }
    this.device = 'PC'
    const hash = {
    ...
    if (osVersion[this.os]) {
      ...
      this.osVersion = this.osVersion.split('.')[0]
      if (this.osVersion == u) {
        this.osVersion = ''
      }
    }
    ...
    if (version[this.browser]) {
      ...
      if (this.version == u) this.version = ''
    }
    //修正
    if (this.browser == 'Edge') {
      ...
      this.engine = 'Blink'
    } else if (this.browser == undefined) {
      this.browser = 'Unknow App'
    }
  }
}
export default (u: string) => new Detect(u)
